...
\subsubsection{State Variables}
imageData ($\mathbb{R}$) \wss{the image data is a single real number?} \wss{The
  LBM calculations output an image?  I would much rather the output is the
  velocity (or whatever CFD parameter is of interest).  This data can then be
  turned into an image by another routine, or simply returned to the user, and
  they can make their own image.  An image as output is not very reusable in
  another context.}
