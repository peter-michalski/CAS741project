...
\begin{minipage}{\textwidth}
	...
	\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
		...
		SI Units & $\frac{m}{s}$ \\
		\hline
		Equation& $\mathrm{e} = \mathrm{e_i}$ \\
		\hline
		Description &
		Velocity $\mathrm{e}$ is discretized into a finite number of directions depending on the chosen model. For example a D2Q9 model has 9 velocity directions, $\mathrm{e_0}$ to $\mathrm{e_8}$.\\
		\hline
		Sources& \citet{gibiansky}\\
		\hline
		Ref.\ By & \\
...
\begin{minipage}{\textwidth}
	...
	\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
		...
		\hline
		Description &
		Velocity distribution $f_{i}$ for the velocity direction $i$ is updated for the next iteration.\\
		\hline
		Sources& \citet{gibiansky}\\
		\hline
		Ref.\ By & \\
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Equation&  $f_{i}^{eq} = pw_{i}[1 + \frac{2\textbf{c}_\textbf{i}\textbf{u}-\textbf{u}\textbf{u}}{2c_s^2}+\frac{(\textbf{c}_\textbf{i}\textbf{u})^2}{2c_s^4}] + O(u^2)$\\
  \hline
  Description &
  ...
  $\rho$ is the fluid density $(\mathrm{\frac{g}{cm^2}}$). $w$ is the weighting
                ...
                directions of the chosen lattice model. $\textbf{c}_\textbf{i}$ is the
...
\\
  ...
  \hline
  Ref.\ By & DD\ref{DD_BGK} A\ref{A_fluids}  A\ref{A_flowObject} A\ref{A_lattice} A\ref{A_weightCoefficients} A\ref{A_selectModel} \\
  \hline
\end{tabular}
\end{minipage}\\
...
\hline
G\ref{G_Location} & DD\ref{DD_BGK} & \checkmark & & & & \checkmark \\
\hline
...
\hline
G\ref{G_Velocity} & DD\ref{DD_BGK} & \checkmark & & & & \checkmark \\
\hline
...
\hline
G\ref{G_FluidPressure} & DD\ref{DD_BGK} & \checkmark & & & & \checkmark \\
\hline
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_Inputs}:] The system shall read a set of input fluid parameters, listed in Table \ref{table:inputVar}. These parameters will be used in calculations for \tref{T_BTE}, DD\ref{DD_BGK}, and \tref{T_EDF}.
...
\hline
& \textbf{\tref{T_BTE} }& \textbf{DD\ref{DD_BGK} } & \textbf{\tref{T_EDF} } &
...
\hline
\textbf{DD\ref{DD_BGK} }& \checkmark & & \checkmark & & & & & &\\
\hline
\textbf{DD\ref{DD_BGK} }& & & \checkmark & & \checkmark & & & & \checkmark \\
\hline
