...
\\
$\eta$ & $Pa-s$ & viscosity \wss{The units are not shown correctly.  (The font
  is incorrect, and the units themselves should be pascal seconds (Pa times s).
  I suggest using the si package, as in the SRS.tex example.}
\\ 
...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_fluids}:]
  ...
  DD\ref{DD_RelaxationRate}, and in DD\ref{DD_VelocityGradient}. \wss{I like the
    traceability, but I don't see how this is a variability.  As far as
    assumptions go, I also do not know what is being said here.}
  
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_flowObject}:]
  The fluid may flow through an object with boundary conditions. \wss{I'm not
    sure of the purpose of this assumption.}
...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_weightCoefficients}:]
  ...
  \ref{coefficientweights}.  \wss{I like this variability.  It isn't really
    connected to the rest of the document, since it doesn't tell me where weight
    coefficients are used, but it does tell me of the discrete set of choices
    that people use for the values.  We can make it a bit better when we add a
    data definition for the discretization.  (I'll be discussing this
    discretization DD separately.)}
...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_selectModel}:] The user
  ...
  vector of DD\ref{DD_RelaxationRate} is influenced by this. \wss{In the \LaTeX,
    you are not using 80 character line widths.  I've fixed this in a few spots,
    but it would be great if you could start working with an editor that
    supports 80 character line widths.}
...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_userInputs}:] The user
  ...
  and DD\ref{DD_FluidDensity}. \wss{I don't think this is an assumption.  It
    sounds like a requirement.}
  
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_speedSound}:] The speed
  of sound is constant. Its value can be found in Section
  \ref{symbolicpara}. The constant is referenced in model \tref{T_EDF}. \wss{The
    speed of sound depends on what material it is moving through.  Do you really
    need to assume it is constant?  The Lattice Boltzmann solver will be using
    different materials.}
  
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_AVGERROR}:]
  The average error in outputs between OTS solutions is 3\%.  \wss{What is an
    OTS solution - I searched the document.  This looks to be a reference to off
    the shelf solutions.  I like the idea of comparing your results to OTS
    solutions.  However, this isn't an assumption, but a nonfunctional
    requirement.  Furthermore, it isn't really a nonfunctional requirement we
    should include.  Rather than a priori specify the average error.  I would
    prefer if we \emph{a posteriori} described the average error.  This should
    be discussed in your verification and validation plan.}
...
\wss{You are on to something with this table, but there are some things that do
  not make sense to me.  Many of the variabilities are inputs, like viscosity,
  force etc.  I think it is cleaner to think of them as inputs, not
  variabilities.  I don't understand the ``boundary shape'' variability.  The
  model choice variability should be relabelled, dimension.  The number of
  velocity directions looks interesting.  This is a variability that should be
  explained.  I'm sure that it depends on the dimension variability, so this
...
\wss{The D1Q2 etc variabilities are great.  You should explain your convention
  for their naming and connect them to the rest of the document.  It should help
  when we add a DD for the velocity discretization.}
...
\wss{I really like what you have in the appendix.  It is obvious you have had a
  careful look at the existing solutions and made an effort to classify them by
  variabilities.  However, you have left out variabilities.  Please add the
  variabilities identified by Mohammad.  I sent you a spreadsheet created by him
  on September 12.  The name of the file is LBM Opensources.xls.  He has
  variabilities related to turbulence, and non-Newtonian fluids.  If you do not
  know the value of a given variability for a given package, you can enter a
